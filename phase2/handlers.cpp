...
ShowPage::ShowPage(string _filePath) { filePath = _filePath; }
...
ShowImage::ShowImage(string _filePath) { filePath = _filePath; }
...
Response *SignupHandler::callback(Request *req) {
  ...
  jeekJeeker.signUp(username , displayName , password);
  res->setHeader("Content-Type", "text/html");
  string body = readFile("htmlFiles/login.html" );
...
Response *SearchHandler::callback(Request *req) {
  ...
  for (int i=0;i<jeeksFound.size();i++)
