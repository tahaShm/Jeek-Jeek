...
void JeekJeek::signUp(string _username , string _displayName , string _password){
...
void JeekJeek::addNewJeek(Jeek newJeek){
	if (is_user_in){
		numOfJeeks ++;
		...
		notifyMentionedUsers(newJeek);
	}
}
void JeekJeek::notifyMentionedUsers(Jeek newJeek){
	vector <string> _mentionedUsers = newJeek.get_mentionedUsers();
	for (int i=0;i<_mentionedUsers.size();i++){
		string newNotification = userIn->getUsername() + " mentioned you in " + newJeek.getId();
...
void JeekJeek::addComment(string _jeekId , string _commentText){
	if (is_user_in){
		int userId = findUserOnJeekId(_jeekId);
		if (userId != INVALID){
			numOfComments ++;
...
void JeekJeek::addReplyToComment(string _commentId , string _replyText){
	if (is_user_in){
...
void JeekJeek::addReplyToReply(string _replyId , string _replyText){
	if (is_user_in){
...
void JeekJeek::reJeek(string _jeekId){
	if (is_user_in){
		int userId = findUserOnJeekId(_jeekId);
		if (userId != INVALID){
...
void JeekJeek::showReply(string _replyId){
	...
	if (reply_id != INVALID){
		...
		if (owner != NULL){
			...
			cout << "Replies:" << endl;
			for (int i=0;i<replies.size();i++){
				//cout << "1:"<<replies[i].is_reply_to_reply() << "     2 :"<<replies[i].is_reply_to_thisReply(_replyId)<<endl;
...
void JeekJeek::like(string _jeekId){
	if (is_user_in){
...
void JeekJeek::dislike(string _jeekId){
	if (is_user_in){
...
void JeekJeek::follow(string _username){
	if (is_user_in){
...
void JeekJeek::unfollow(string _username){
	if (is_user_in){
		User* followed = getUserOnName(_username);
		if (followed != NULL){
