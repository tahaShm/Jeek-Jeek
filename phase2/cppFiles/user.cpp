...
string User::checkJeekDetails(string jeekDetail){
	for (int i=0;i<jeeks.size();i++)
		if (jeeks[i].getDetails() == jeekDetail) return jeeks[i].getId();
	return "";
}
string User::get_jeek_datails(string _jeekId){
	for (int i=0;i<jeeks.size();i++)
		if (jeeks[i].getId() == _jeekId) return jeeks[i].getDetails();
	return "";
}
Jeek* User::get_jeek(string _jeekId){
	for (int i=0;i<jeeks.size();i++)
		if (jeeks[i].getId() == _jeekId) return &jeeks[i];
	return NULL;
}
bool User::is_new_liker(string _jeekId , string _username){
	for (int i=0;i<jeeks.size();i++)
		if (jeeks[i].getId() == _jeekId) return jeeks[i].is_liker_unique(_username);
	return true;
}
int User::getJeekLikes(string _jeekId){
	for (int i=0;i<jeeks.size();i++)
		if (jeeks[i].getId() == _jeekId) return jeeks[i].getLikes();
	return 0;
}
int User::getJeekRejeeks(string _jeekId){
	for (int i=0;i<jeeks.size();i++)
		if (jeeks[i].getId() == _jeekId) return jeeks[i].getRejeeks();
	return 0;
}