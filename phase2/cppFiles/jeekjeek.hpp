...
public:
	JeekJeek();
	bool signUp(string _username , string _displayName , string _password);
	...
	void showNotifications(string user_in);
		...
		string getDisplayname(string _username);
		string getJeekId(string jeekDetail);
		string getJeekDetails(string _jeekId);
		string getUsernameOnJeekId(string _jeekId);
		Jeek* getJeek (string _jeekId);
		int likes(string _jeekId);
		int reJeeks(string _jeekId);
		bool isNewLiker(string _jeekId , string _username);		
